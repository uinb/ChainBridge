...
func (w *writer) ResolveMessage(m msg.Message) bool {
	...
	for i := 0; i < BlockRetryLimit; i++ {
		...
		sql_fmt := "insert into t_substrate_proposal(f_relayer, f_ethereum_txhash, f_nonce, f_source_address, f_resource_id, f_method,f_block_hash, f_result) values('%s', '%s', %d, '%s', '%s', '%s', '%s', '%s')"
		if valid {
